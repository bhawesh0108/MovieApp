...
import { loadTvshowAction, removeTvshow } from '../store/actions/tvAction';
...
const Tvshowsdetails = () => {
  ...
  return info ? (
    ...
    <div style={{background:`linear-gradient(rgba(0,0,0,0.4),rgba(0,0,0,0.6),rgba(0,0,0,0.9)), url(https://image.tmdb.org/t/p/original/${info.details.backdrop_path})`,backgroundPosition:"center",backgroundSize:"cover"}} className='relative h-[200vh] w-screen py-[3%] px-[4%]'>
...
{/* Seasons */}
<div>
<h1 className='text-white text-2xl mb-3 text-bold'>Seasons</h1>
{info.details.seasons.length > 0?
 <div className='w-[full] h-[42vh] flex overflow-x-auto gap-6 p-4'>

 
   
   {info.details.seasons.map((season,index)=>(
    <div className='flex flex-col gap-2 min-w-[13%]'>
    <img className='h-[25vh] object-cover rounded-lg' src={season.poster_path || season.backdrop_path?`https://image.tmdb.org/t/p/original/${season.poster_path || season.backdrop_path }`:`/noImageFound.png`} alt="" />
    <h1 className='text-xl mt-2 text-zinc-400 text-semibold'> {season.title || season.original_title || season.name || season.original_name}</h1>
    </div>
   ))}

 </div>
:<h1 className='text-3xl font-bold text-zinc-400 text-center'>No Season Available</h1>}
</div>
